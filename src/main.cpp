...
vector <Cell*> vc, *bestvc;  //vc: vector of cell
...
void updateGain(Cell * c){
    ...
    c->lock = true;
    int num = c->to->id;
    cellstack.push_back(num);  //restore best situation
    if (!c->set) {//Cell in A _DC  move cell: A->B _DC
        ...
        for(int i = 0; i < szn; i++){
            ...
            net->B++;
            c->set = true;
            //update other cell's gain in the same net
            if (net->A == 0){
                for (int j = 0; j < szc; j++){
                    int idc = net->cellList[j];
                    if (!vc[idc]->lock) {
